></div>";<?php
  ...
  try
  {
    ...
    $db->exec("CREATE TABLE IF NOT EXISTS users (firstname TEXT NOT NULL,lastname TEXT NOT NULL,email TEXT NOT NULL,nationality TEXT NOT NULL,regdate TIMESTAMP NOT NULL,custom1 TEXT,custom2 TEXT,custom3 TEXT,custom4 TEXT);");
    $db->exec("CREATE VIEW IF NOT EXISTS reglog AS SELECT firstname, lastname, email, nationality, strftime('%d/%m/%Y %H:%M:%S', datetime(regdate/1000, 'unixepoch', 'localtime')) regdatetime FROM users;");
    $firstname = $_POST["firstname"];
    $lastname = $_POST["lastname"];
    $email = $_POST["email"];
    $nationality = $_POST["nationality"];
    $regdate = intval($_POST["regdate"]);
    $dbq = $db->prepare("INSERT INTO `users` (firstname,lastname,email,nationality,regdate) VALUES (:firstname,:lastname,:email,:nationality,:regdate);");
    $dbq->execute(['firstname'=>$firstname,'lastname'=>$lastname,'email'=>$email,'nationality'=>$nationality,'regdate'=>$regdate,]);
    $db = NULL;
  ...
  try
  {
    ...
    $mail->setFrom("f1simulator@elementz.live","Elementz Group");
    $mail->addReplyTo("noreply@elementz.live", "Elementz F1 app");
    $mail->isHTML(true);
    ...
    $mail->Subject = "F1 Simulator Registration";
    $mail->Body = "<div dir='ltr' lang='en'>Hello {$_POST['firstname']} {$_POST['lastname']},<br/><br/>Thank you for registering to participate in the F1 simulator experience.<br/><br/>Attached is your personalized QR code for admission.<br/><br/>Best regards,<br/><br/>Elementz Group<br/><br/></div><div><img src='cid:qrpng' alt='' /><br/><img src='cid:ralogo' alt='' /><br/></div>";
    $mail->send();
  }
    catch (Exception $e)
  {
